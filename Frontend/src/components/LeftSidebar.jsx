...
import { Home, LogOut, MessageCircle, PlusSquare, AppWindow, Leaf, Languages, FileText, LayoutDashboard } from "lucide-react"
...
const LeftSidebar = () => {
  ...
  const sidebarHandler = (textType) => {
    if (textType === "MagicMatching") {
      navigate("/magic-matching")
    } else if (textType === "Volunteer Dashboard") {
    ...
    else if (textType === "Your Events") {
      navigate("/your-events")
    }
    else if (textType === "Smart AI") {
  ...
  const sidebarItems = [
    ...
    // Conditionally show dashboard options based on user type
    ...(user?.type === "ngo"
      ? [
          { icon: <Calendar className="text-green-600" />, text: "Create Event" },
          { icon: <AppWindow className="text-green-600" />, text: "Dashboard" },
          { icon: <Calendar className="text-green-600" />, text: "Your Events" }  // ← ADD THIS
        ]
      : user?.type === "volunteer"
      ? [
          { icon: <Calendar className="text-green-600" />, text: "Explore Events" },
          { icon: <Leaf className="text-green-600" />, text: "MagicMatching" },
        ]
      : []),
    ...
    { icon: <FileText className="text-green-600" />, text: "Smart AI" },
